...
<template>
    <div class="col-span-full sm:col-span-6 xl:col-span-4 bg-white shadow-lg rounded-sm border border-slate-200">
        <div class="flex flex-col h-full p-5">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex shrink-0 -space-x-3 -ml-px">
                        <router-link
                            ...
                            :to="member.link"
                        >
                            <img class="rounded-full border-2 border-white box-content" :src="member.image" width="28"
                                 height="28" :alt="member.name"/>
                        </router-link>
                    </div>
                </div>
            </header>
            ...
            <footer class="mt-5">
                <div class="text-sm font-medium text-slate-500 mb-2">{{ item.dates.from }} <span class="text-slate-400">-&gt;</span>
                    {{ item.dates.to }}
                </div>
                <div class="flex justify-between items-center">
                    <div>
                        <div class="text-xs inline-flex font-medium rounded-full text-center px-2.5 py-1"
                             :class="typeColor(item.type)">{{ item.type }}
                        </div>
                    </div>
                    <div>
                        <router-link class="text-sm font-medium text-indigo-500 hover:text-indigo-600" :to="item.link">
                            View -&gt;
                        </router-link>
                    </div>
                </div>
